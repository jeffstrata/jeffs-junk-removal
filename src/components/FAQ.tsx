          ...
          {faqs.map((faq, index) => (
            <div
              ...
              className={`bg-white rounded-2xl border overflow-hidden transition-all duration-300 ${
                openIndex === index ? 'border-accent/30 shadow-lg shadow-accent/5' : 'border-gray-100'
              }`}
            >
              <button
              ...
              >
                <span className="text-base font-bold text-navy pr-4">{faq.question}</span>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-300 ${
                  openIndex === index ? 'bg-accent text-white rotate-180' : 'bg-gray-100 text-gray-500'
                }`}>
